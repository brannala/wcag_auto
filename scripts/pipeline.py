...
# Ensure scripts directory is on the import path for html_auto_fixes
...
class ClaudeRemediationGenerator:
    ...
    def generate_remediation_input(
        ...
        for file_report in html_report.get('file_reports', []):
            ...
            for issue in file_report.get('issues', []):
                ...
                if categorized.get('suppressed'):
                    remediation_tasks['summary']['suppressed_notices'] += 1
                elif categorized['auto_fixable']:
    ...
    def _categorize_html_issue(self, issue: dict) -> dict:
        ...
        type_code = issue.get('typeCode', 1)
        ...
        # Structural/landmark issues — auto-fixable
        if code in ('landmark-one-main', 'region'):
            category = 'structure'
            auto_fixable = True
            remediation_hint = 'Wrap body content in <main> landmark'

        elif code == 'aria-hidden-focus':
            category = 'aria'
            auto_fixable = True
            remediation_hint = 'Add tabindex="-1" to focusable elements in aria-hidden containers'

        elif code == 'listitem':
            category = 'lists'
            auto_fixable = True
            remediation_hint = 'Wrap orphaned <li> elements in <ul>'

        elif code == 'page-has-heading-one':
            category = 'headings'
            auto_fixable = True
            remediation_hint = 'Add h1 from page title'

        elif 'H42' in code:
            category = 'headings'
            auto_fixable = True
            remediation_hint = 'Convert <p><strong> faux headings to proper heading tags'

        elif code == 'html-has-lang' or 'H57' in code:
            category = 'language'
            auto_fixable = True
            ...
            category = 'structure'
            auto_fixable = True
            remediation_hint = 'Ensure page has a descriptive title'

        elif 'DataTable' in code or 'NoCaption' in code or code == 'th-has-data-cells':
            category = 'tables'
            auto_fixable = True
            remediation_hint = 'Add scope attributes to <th> and <caption> to tables'

        # Content issues requiring judgment
        elif 'alt' in code.lower() or 'image' in message.lower():
            category = 'images'
            auto_fixable = False
            remediation_hint = 'Generate descriptive alt text based on image content and context'

        elif 'link' in code.lower() or 'anchor' in message.lower() or 'H91' in code:
            category = 'links'
            auto_fixable = 'empty' in message.lower()
            remediation_hint = 'Make link text descriptive of destination'

        elif 'heading' in code.lower() or 'G141' in code:
            category = 'headings'
            auto_fixable = False
            remediation_hint = 'Ensure proper heading hierarchy (h1 > h2 > h3)'

        elif 'lang' in code.lower() or 'language' in message.lower() or 'H58' in code:
            category = 'language'
            auto_fixable = True
            remediation_hint = 'Add lang attribute to html element'
        ...
        # Suppress generic HTMLCS notices that fire on every page
        # typeCode 3 = notice, and 1.4.10 reflow on <pre> blocks
        if type_code == 3 and not auto_fixable:
            suppressed = True
            category = 'notice'
            remediation_hint = 'Manual review notice — no automated fix applicable'

        if '1_4_10' in code:
            suppressed = True
            category = 'notice'
            auto_fixable = False
            remediation_hint = 'Pre-formatted code blocks are acceptable reflow exceptions per WCAG 2.1 SC 1.4.10'
...
class HTMLRemediator:
    ...
    def apply_auto_fixes(self, content_dir: Path, remediation_input: dict, output_dir: Path) -> dict:
        ...
        Uses phased fix functions from html_auto_fixes module.
        Returns report of applied fixes.
        """
        from html_auto_fixes import apply_all_fixes
        ...
        for task in remediation_input.get('html_tasks', []):
            ...
            if applied:
                ...
                fixes_applied['total_fixes'] += len(applied)
