...
**Note:** Some institutions (e.g., UC Davis) do not grant faculty access to the Canvas API. If your institution restricts API access, use the manual export/import workflow described below instead.

For institutions that do provide API access, relevant endpoints include:

```bash
GET /api/v1/courses/:course_id/pages          # List all pages
GET /api/v1/courses/:course_id/pages/:url      # Get page content
PUT /api/v1/courses/:course_id/pages/:url      # Update page content
GET /api/v1/courses/:course_id/files           # List all files
```
